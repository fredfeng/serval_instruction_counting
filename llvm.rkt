...
(struct machine (frames mregions retval merge-stack merge-point instr-count) #:mutable #:transparent)
...
(define (make-machine [symbols null] [globals null])
  ...
  (machine (list (make-empty-frame)) mregions %0 null #f 0))
...
(define (current-instr-count)
  (machine-instr-count (current-machine)))

(define (incr-current-instr-count!)
  (set-machine-instr-count! (current-machine) (+ 1 (current-instr-count))))
